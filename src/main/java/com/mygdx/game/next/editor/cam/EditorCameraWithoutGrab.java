...
public class EditorCameraWithoutGrab extends GestureDetector {
  ...
  public int strafeRightKey = Keys.A;
  protected boolean strafeRightPressed;
  public int strafeLeftKey = Keys.D;
  ...
  protected static class CameraGestureListener extends GestureAdapter {
    public EditorCameraWithoutGrab controller;
  ...
  protected EditorCameraWithoutGrab(
  ...
  public EditorCameraWithoutGrab(final Camera camera) {
  ...
  public void update() {
    if (strafeRightPressed || strafeLeftPressed || forwardPressed || backwardPressed) {
      ...
      if (strafeRightPressed) {
        camera.translate(
            tmpV1.set(camera.up).crs(camera.direction).nor().scl(delta * translateUnits));
      }
      if (strafeLeftPressed) {
        camera.translate(
            tmpV1.set(camera.direction).crs(camera.up).nor().scl(delta * translateUnits));
      }
  ...
  protected boolean process(float deltaX, float deltaY, int button) {
    if (button == rotateButton) {
      ...
      tmpV1.nor();
      camera.rotate(tmpV1, deltaY * rotateAngle);
